...
export function ResultCard({ title, mainValue, mainLabel, items, visible, children }: ResultCardProps) {
  ...
  return (
    <div 
      className={`mt-6 rounded-xl bg-gradient-to-br from-navy to-navy-light dark:from-navy-dark dark:to-navy p-6 text-white shadow-lg border border-white/5 relative transition-all duration-500 print:bg-none print:text-navy print:border-navy print:shadow-none print:mt-0 ${animate ? 'scale-[1.02] ring-2 ring-accent/50 shadow-accent/20' : 'scale-100'}`} 
      aria-live="polite"
    >
      {/* Branded print header */}
      <div className="hidden print:flex items-center justify-between border-b border-navy/10 pb-4 mb-6">
        <span className="text-xl font-bold text-navy">{t.brand}</span>
        <span className="text-xs text-slate-500">{new Date().toLocaleDateString(isSpanish ? 'es-ES' : 'pt-BR')}</span>
      </div>

      <div className="flex justify-between items-start">
        <div className="flex-1 mr-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 print:text-slate-500 mb-1">{title}</p>
          <p className="text-4xl font-bold break-all print:text-navy">{mainValue}</p>
          <p className="text-sm text-slate-300 font-medium print:text-slate-600">{mainLabel}</p>
        </div>
      ...
      {items && items.length > 0 && (
        <div className="mt-4 space-y-2 border-t border-white/20 print:border-navy/10 pt-4">
          {items.map((item) => (
            <div key={item.label} className="flex justify-between text-sm">
              <span className="text-slate-300 font-medium print:text-slate-600">{item.label}</span>
              <span className={item.highlight ? 'font-bold text-white print:text-navy' : 'text-slate-200 print:text-slate-800'}>{item.value}</span>
            </div>
          ))}
        </div>
      )}
