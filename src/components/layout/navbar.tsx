    ...
    <header className="sticky top-0 z-50 border-b border-navy-light bg-navy text-white">
      <div className="container-app flex h-14 items-center justify-between">
        ...
        <div className="flex items-center gap-1 md:hidden">
          <ThemeToggle />
          <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 text-slate-300 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-inset rounded-lg" aria-label="Menu" aria-expanded={menuOpen}>
      ...
      {menuOpen && (
        <nav className="md:hidden border-t border-navy-light bg-navy px-4 pb-4" aria-label="Navegação por categorias">
          {CATEGORIAS.map((cat) => (
            <Link key={cat.slug} href={`/${cat.slug}`} onClick={() => setMenuOpen(false)} className="block py-3 text-sm text-slate-300 hover:text-white min-h-[44px] flex items-center">{cat.nome}</Link>
          ))}
        </nav>
      )}
    </header>
