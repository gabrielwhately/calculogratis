  ...
  return (
    <div className="relative">
      <input type="search" placeholder="Buscar calculadora..." value={query}
        ...
        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 outline-none focus:border-accent focus:ring-2 focus:ring-blue-100 md:w-64" />
        ...
        <div className="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg border border-slate-200 bg-white shadow-lg">
          {results.map((calc) => (
            <Link key={calc.slug} href={`/${calc.categoriaSlug}/${calc.slug}`}
              className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50" onClick={() => setIsOpen(false)}>
              <span className="font-medium">{calc.nome}</span>
              <span className="ml-2 text-xs text-slate-500">{calc.categoria}</span>
            </Link>
          ))}
        </div>
      )}
